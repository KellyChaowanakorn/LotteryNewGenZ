...
export default function Home() {
  ...
  // Tab labels with language support
  const tabLabels = {
    lottery: language === "th" ? "1. หวย" : "1. Lottery",
    bet: language === "th" ? "2. แทง" : "2. Bet",
    number: language === "th" ? "3. เลข" : "3. Number",
    amount: language === "th" ? "4. เงิน" : "4. Amount"
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold">
              {language === "th" ? "เครื่องคิดเลขแทงหวย" : "Lottery Calculator"}
            </h1>
            <p className="text-sm text-muted-foreground">
              {language === "th" 
                ? "หวย 6 ประเภท: รัฐบาล, หุ้นไทย, นิเคอิ, ฮั่งเส็ง, ดาวโจนส์, มาเลย์" 
                : "6 Types: Thai Gov, SET, Nikkei, HSI, Dow, Malaysia"}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
              <TabsList className="grid w-full grid-cols-4">
                <TabsTrigger value="lottery">{tabLabels.lottery}</TabsTrigger>
                <TabsTrigger value="bet" disabled={!selectedLottery}>{tabLabels.bet}</TabsTrigger>
                <TabsTrigger value="number" disabled={!selectedBetType}>{tabLabels.number}</TabsTrigger>
                <TabsTrigger value="amount" disabled={numbers.length === 0}>{tabLabels.amount}</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="lottery">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {language === "th" ? "เลือกหวย 6 ประเภท" : "Select from 6 lottery types"}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    ...
                    {selectedLottery && (
                      <Button className="w-full mt-4" onClick={goToNextTab}>
                ...
                <Card>
                  ...
                  <CardContent>
                    <BetTypeSelector 
                      ...
                      onSelect={(type) => { setSelectedBetType(type); setNumbers([]); }} 
                      lotteryType={selectedLottery} 
                    />
                    {selectedBetType && (
                      <Button className="w-full mt-4" onClick={goToNextTab}>
              ...
              <CardContent className="space-y-3">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{amount.toLocaleString()} {t("common.baht")}</span>
                  </div>
                </div>
